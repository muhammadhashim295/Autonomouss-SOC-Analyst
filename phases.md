...
Each step is scoped to be independently buildable and verifiable before moving to the next. Feed these to Qoder one at a time, confirming the verify condition before proceeding.
...
**5. Qoder Agent + Environment setup (Primary)**
Define Alert Triage Agent, its Environment, Vault (OTX key), scoped IAM (read-only + documentation-write, no execution).
Verify: session created, agent responds to a manually passed alert with basic reasoning.
...
**8. Qoder Agent setup (Secondary)**
Define Deep Investigation Agent, its own Environment, IAM scoped for standard-action execution only (no high-impact permissions).
Verify: session created, can read primary's output.
...
**11. Memory store + case retrieval + learning**
Create memory store, attach to both agents' sessions. Write structured case records on close; retrieve similar past cases before new investigations.
...
Backend relays live session events (agent reasoning as it happens) to frontend via SSE.
...
"Generate Live Feed" button in the dashboard near the alert queue. On click: calls `POST /alerts/generate/start` (default 120 s, 5 s interval, 15% poison ratio). Button shows active/running state (disabled + "Generating..." + countdown) for the duration, then reverts. New alerts appear in the alert queue in near-real-time via polling (upgradeable to SSE later). Wired to Phase 3b backend endpoints.
